...
const useMarvelService = () => {
  ...
  const getAllCharacters = async (offset = _offsetCharacters) => {
    const res = await request(
      `${_apiBase}/characters?limit=${_limitCharacters}&offset=${offset}&apikey=${_apiKey}`
    );
  ...
  const getCharacterByName = async (name) => {
    const res = await request(
      `${_apiBase}/characters?name=${decodeURI(name)}&apikey=${_apiKey}`
    );

    if (res.data.results.lengh === 0) {
  ...
  const getAllComics = async (offset = _offsetComics) => {
    const res = await request(
      `${_apiBase}/comics?orderBy=issueNumber&limit=${_limitComics}&offset=${offset}&apikey=${_apiKey}`
    );
  ...
  const _transformCharacter = (char) => {
    return {
      ...
      name: char.name,
      description:
        extractTextContent(char.description) || "There is no any description.",
      thumbnail: char.thumbnail.path + "." + char.thumbnail.extension,
      imageNotFound: char.thumbnail.path.includes(
        "/u/prod/marvel/i/mg/b/40/image_not_available"
      ),
      homepage: char.urls[0].url,
      wiki: char.urls[0].url,
      comics: char.comics.items.map((comic) => {
        comic.id = comic.resourceURI.substring(
  ...
  const _transformComics = (comics) => {
    return {
      ...
      imageNotFound: comics.thumbnail.path.includes(
        "/u/prod/marvel/i/mg/b/40/image_not_available"
      ),
  ...
  const _transformSingleComic = (comic) => {
    return {
      ...
      imageNotFound: comic.thumbnail.path.includes(
        "/u/prod/marvel/i/mg/b/40/image_not_available"
      ),
